...
interface Document {
  ...
  url: string | null; tags: string[]; created_at: string; updated_at: string | null;
}
...
export default function DocumentsContent() {
  ...
  async function deleteFolder(id: string) {
    ...
    if (res.ok) {
      ...
      setToast({ message: "Folder dihapus", type: "success" }); fetchDocs();
    }
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 h-full">
      <ConfirmModal
        ...
        message={deleteTarget?.type === "folder" ? "Folder akan dihapus. Dokumen dipindahkan ke Tanpa Folder." : "Dokumen dihapus permanen."}
      ...
      <aside className="w-full md:w-48 shrink-0 flex flex-col gap-1">
        <button onClick={selectAll}
          className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-colors text-left w-full ${!showUnfoldered && selectedFolder === null ? "bg-neutral-200 dark:bg-neutral-700 text-neutral-800 dark:text-neutral-200 font-semibold" : "text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800"}`}>
          <FileText size={15} /> Semua Dokumen
        </button>
        <button onClick={selectUnfoldered}
          className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-colors text-left w-full ${showUnfoldered ? "bg-neutral-200 dark:bg-neutral-700 text-neutral-800 dark:text-neutral-200 font-semibold" : "text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800"}`}>
        ...
        {folders.map(folder => {
          ...
          return (
            <div key={folder.id} onClick={() => selectFolder(folder.id)}
              className={`group flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium cursor-pointer transition-colors ${isSelected ? `${bgClass} ${textClass} font-semibold` : "text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800"}`}>
        ...
        <button onClick={openNewFolder}
          className="mt-2 flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button onClick={openNewDoc}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold bg-neutral-500 hover:bg-neutral-600 text-white transition-colors shrink-0">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => handleSearch(e.target.value)} placeholder="Cari dokumen..."
            className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white dark:bg-[var(--bg-canvas)] border border-gray-200 dark:border-gray-700 text-sm focus:ring-2 focus:ring-neutral-400 dark:focus:ring-neutral-600 outline-none" />
        </div>
        ...
        {docs.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
            ...
            {!search && (
              <button onClick={openNewDoc} className="mt-4 flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold bg-neutral-500 hover:bg-neutral-600 text-white transition-colors">
