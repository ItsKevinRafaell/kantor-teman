...
export default function FollowUpModal({ lead, open, message, templates, onClose, onMessageChange, onSend, onSelectTemplate }: FollowUpModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 w-full max-w-md p-6 space-y-4">
        <h3 className="text-sm font-bold text-neutral-800 dark:text-neutral-200">Follow Up: {lead.business_name}</h3>
        {templates.length > 0 && (
          <div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={onSend} className="px-4 py-2 text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white rounded-xl transition-colors">Kirim via WA</button>
        </div>
      </div>
    </div>
  );
