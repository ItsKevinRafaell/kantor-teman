...
export default function EditClientModal({ contact, open, onClose, onSuccess, setToast }: EditClientModalProps) {
  ...
  async function handleSave() {
    ...
    if (res.ok) {
      ...
      setToast({ message: "Gagal memperbarui klien.", type: "error" });
    }
  }
      ...
      <div className="relative bg-[var(--bg-surface)] rounded-2xl shadow-2xl border border-[var(--border-default)] w-full max-w-md p-6 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={form.business_name} onChange={e => setForm(f => ({ ...f, business_name: e.target.value }))} className={inputCls} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Nomor WhatsApp</label>
            <input value={form.phone_number} onChange={e => setForm(f => ({ ...f, phone_number: e.target.value }))} className={inputCls} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.purchased_product} onChange={e => setForm(f => ({ ...f, purchased_product: e.target.value }))} className={inputCls} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Catatan</label>
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3} className={inputClsLarge} />
          </div>
        </div>
