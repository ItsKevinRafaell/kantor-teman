...
class ReportNotDraftError(ValueError):
    pass


class ReportNoChangesError(ValueError):
    pass
...
def patch_draft_report_snapshot(
    db: Session,
    *,
    report_id: str,
    manual_metrics: Optional[dict],
    ...
    """Revise report contents in the existing draft snapshot only.

    This deliberately does not share the generation/finalization path: no
    snapshot dedup lookup, invoice work, notification, email, or publication
    state is touched. PageSpeed is always disabled for an edit.
    """
    snapshot = db.query(ReportSnapshot).filter(ReportSnapshot.id == report_id).first()
    if not snapshot:
        raise ReportNotFoundError("Laporan tidak ditemukan")
    if (snapshot.status or "").lower() != "draft":
        raise ReportNotDraftError("Hanya laporan draft yang dapat diubah")

    document = None
    if snapshot.generated_document_id:
        document = db.query(GeneratedDocument).filter(GeneratedDocument.id == snapshot.generated_document_id).first()
    if not document:
        raise ValueError("Dokumen draft laporan tidak ditemukan")

    existing_metrics = _json_loads(snapshot.metrics_json, {})
    current_manual_metrics = _clean_dict(existing_metrics.get("manual"))
    current_evidence = _json_loads(snapshot.evidence_json, {})
    current_editable_evidence = {key: value for key, value in current_evidence.items() if key != "workspace_evidence"}
    current_narrative = _json_loads(snapshot.narrative_json, {})

    # PATCH input updates individual keys, matching the evidence/narrative
    # semantics. This prevents a one-field correction from erasing the rest of
    # the manually-entered metrics.
    revised_manual_metrics = dict(current_manual_metrics)
    if manual_metrics is not None:
        revised_manual_metrics.update(_clean_dict(manual_metrics))

    # workspace_evidence is generated by the server and intentionally cannot be
    # changed by a client PATCH. Excluding it before comparison also prevents a
    # managed-only request from pretending to be an effective content change.
    requested_editable_evidence = {
        key: value
        for key, value in _clean_dict(evidence).items()
        if key != "workspace_evidence"
    }
    revised_editable_evidence = dict(current_editable_evidence)
    if evidence is not None:
        revised_editable_evidence.update(requested_editable_evidence)

    revised_narrative = dict(current_narrative)
    if narrative is not None:
        revised_narrative.update(_clean_dict(narrative))

    changed_fields = []
    if manual_metrics is not None and revised_manual_metrics != current_manual_metrics:
        changed_fields.append("metrics")
    if evidence is not None and revised_editable_evidence != current_editable_evidence:
        changed_fields.append("evidence")
    if narrative is not None and revised_narrative != current_narrative:
        changed_fields.append("narrative")
    if not changed_fields:
        raise ReportNoChangesError("Tidak ada konten laporan yang berubah")

    # Revisions are based on the persisted generation payload, rather than live
    # board/workspace/project records, so an edit preserves report snapshot data.
    payload = _json_loads(document.variables_used, {})
    if not isinstance(payload, dict) or not payload:
        raise ValueError("Snapshot dokumen laporan tidak valid")
    payload["title"] = snapshot.title
    payload["report_type"] = snapshot.report_type
    payload["target_type"] = snapshot.target_type
    payload["target_id"] = snapshot.target_id

    metrics = _clean_dict(payload.get("metrics"))
    metrics.update(existing_metrics)
    metrics["manual"] = revised_manual_metrics
    service_type = payload.get("service_type") or snapshot.service_type or "general"
    metrics["service"] = _manual_service_metrics(service_type, revised_manual_metrics)
    metrics["comparisons"] = _derive_comparisons(metrics, service_type, snapshot.report_type)
    metrics["next_month_targets"] = _derive_next_month_targets(metrics, service_type, snapshot.report_type)
    metrics["comparison_groups"] = _derive_comparison_groups(revised_manual_metrics)
    payload["metrics"] = metrics

    revised_evidence = dict(revised_editable_evidence)
    revised_evidence["workspace_evidence"] = current_evidence.get("workspace_evidence", [])
    payload["evidence"] = revised_evidence

    payload["narrative"] = revised_narrative

    # Only content/document fields are updated. Identity, target, period,
    # publication, finalization, invoice state, and original report title remain untouched.
    snapshot.metrics_json = json.dumps(payload["metrics"], ensure_ascii=False)
    snapshot.evidence_json = json.dumps(payload["evidence"], ensure_ascii=False)
    snapshot.narrative_json = json.dumps(payload["narrative"], ensure_ascii=False)
    snapshot.updated_at = _now()
    ...
    old_url = document.file_url or ""
    pdf_filename = f"{uuid.uuid4()}.pdf"
    pdf_path = os.path.join(DOCUMENTS_DIR, pdf_filename)
    new_url = f"/uploads/generated_documents/{pdf_filename}"
    try:
        # The write itself must be protected: open/write can leave a partial
        # replacement file before raising, while the old document must remain
        # the committed version.
        with open(pdf_path, "wb") as pdf_file:
            pdf_file.write(pdf_bytes)
        document.variables_used = json.dumps(payload, ensure_ascii=False)
        document.file_url = new_url
        document.display_filename = document.display_filename or snapshot.title
        # generated_by is original generator attribution, not editor attribution.
        for archive in db.query(Document).filter(
            Document.source_type == "generated_document",
        ...
        log_audit(
            ...
            "report_snapshots",
            snapshot.id,
            {"reason": reason.strip(), "changed_fields": changed_fields},
            commit=False,
        )
        db.commit()
    except Exception:
        ...
        try:
            ...
            print(f"[REPORT_DRAFT_PATCH] gagal bersihkan PDF baru: {cleanup_exc}", flush=True)
        raise

    # Do not remove the old file until the replacement URL is committed.
    if old_url and old_url != new_url:
        ...
        try:
            ...
            print(f"[REPORT_DRAFT_PATCH] gagal hapus PDF lama: {exc}", flush=True)
    db.refresh(snapshot)
    return snapshot
